...
export function addDefaultPrefixToPropTypes(propTypes: Object) {
  return _.mapKeys(
    propTypes,
    (value, key) => `default${key.substr(0, 1).toUpperCase()}${key.substr(1)}`,
  );
}
...
function registerEventsFromComponent(
  component,
...
const enhanceWithPropTypes = _.curry(
  (getInstanceFromComponent, controlledPropUpdaterMap, componentSpec) => {
    const { componentDidUpdate = _.noop } = componentSpec;
...
const enhanceWithEventMap = _.curry(
  (getInstanceFromComponent, eventMap, componentSpec) => {
    const {
...
const enhanceWithPublicMethod = _.curry(
  (getInstanceFromComponent, publicMethodMap, componentSpec) =>
    _.reduce(
      publicMethodMap,
